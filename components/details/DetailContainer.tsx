...
function DetailContainer(data: DetailContainerProps) {
  ...
  useEffect(() => {
    ...
    // Fetch trailer when component mounts or ID changes
    if (id && data?.media_type) {
      fetchTrailer();
    }
  }, [id, data?.media_type]); // Dependencies that should trigger a re-fetch
  ...
  const fetchTrailer = async () => {
    try {
      ...
      // Check for both Trailer and Teaser types
      let videos = res.results || [];
      let trailerVideo = videos.find((v: any) => v.type === "Trailer") || 
                        videos.find((v: any) => v.type === "Teaser") ||
                        videos[0]; // Fallback to first video if no trailer/teaser
  ...
  return (
    <>
          ...
          <motion.div  className="flex flex-col pl-4 w-full  md:w-2/3 xl:w-2/5	 justify-center datas-start gap-2 h-full">
            ...
            <div className="flex gap-3 mt-3">
              <button
                ...
                disabled={!trailer}
                className={`py-3 px-5 ${!trailer ? 'bg-gray-600 cursor-not-allowed' : 'bg-red-600 hover:text-black hover:bg-gray-200 hover:scale-105'} text-gray-200 rounded-sm font-semibold flex items-center gap-2 transition-all duration-500 easeInOut`}
              >
      ...
      {isTrailer && trailer && <Trailer trailer_key={trailer} handleClose={handleClose} />}
    </>
  );
}
